...
"""
Creation de la matrice de Vigenere
Cree une matrice de taille 26x26 avec les positions des lettres de l'alphabet
Au lieu de contenir les lettre, la matrice contient la position de la lettre dans l'alphabet en commençant par 0
0 -> A, 1 -> B, ......, 25 -> Z
...
def elongate_key(text, key):
    ...
    key_duplications = 1
    if key_len < text_len:
        # Nombre de fois que la cle doit etre dupliquee
...
"""
Dechiffrement a l'aide de la matrice
"""
def encrypt_or_decrypt_with_matrix(text, key, is_action_to_encrypt=True):
    text = text.upper()
    key = key.replace(" ", "").upper()
    result = ""
    # Selection de l'operation de chiffrement ou de dechiffrement
    ...
    # Allongement de la cle
    elongated_key = elongate_key(text, key)
    for i in range (len(text)):
        char_code = ord(text[i])
        # Cas pour les caracteres non speciaux
        if char_code >= char_a_code and char_code <= char_z_code:
            # Passage a un index entre 0 et 25 pour le texte a chiffrer ou a dechiffrer
            text_char_code = char_code - char_a_code
            # Passage a un index entre 0 et 25 pour la cle
            key_char_code = ord(elongated_key[i]) - char_a_code
            # Recuperation du code ASCII depuis la matrice
            new_char_code = math.floor(vigenere_operation(vigenere_matrix, text_char_code, key_char_code))
            # Passage en caractere a partir du code de la table ASCII
            new_char = chr(new_char_code + char_a_code)
            # Ajout du caractere au resultat
            result += new_char
        else:
            result += text[i]
    return result
...
"""
Dechiffrement a l'aide de la formule mathematique
"""
def encrypt_or_decrypt_mathematically(text, key, is_action_to_encrypt=True):
    ...
    for i in range(text_len):
        ...
        if char_code >= char_a_code and char_code <= char_z_code:
            ...
            # Utilisation de la formule mathematique pour la resolution
